...
export default class SuccessfulEntryThreeQQView extends PopBaseScene {
    public className_key = "SuccessfulEntryThreeQQView";
    ...
    constructor() {
        ...
        this.skin = 'game/uiView/settlement/SuccessfulEntryThreeQQView.json';
    }
    ...
    /**接受奖励 */
    private sucRecvAward(isDouble: boolean = false) {
        SoundMgr.getInstance().playEffect("button", 1);
        ...
        if (ConfigMgr.getInstance().isWeCatMiniGame()) {
            ...
            if (BaseConst.infos.gameInfo.openPsAward == 1 && BaseConst.infos.gameInfo.for_pay == 1) {
                ...
                Laya.timer.once(1000, this, () => {
                    ...
                    MiniGameMgr.instance.showBanner({});
                    //按钮滑动动制定位置
                    Laya.Tween.to(this.btnNextLevel, { bottom: this.nBtNextLevel }, 500, null);
                })
                return;
            } else {
