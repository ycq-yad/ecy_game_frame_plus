...
export default class AnimationMgr {
    ...
    private static _ins: AnimationMgr;
    public static get instance(): AnimationMgr {
     ...
     * @param url 
     */
    public getAtlasAnimationa(url: string, fex: string): Promise<Laya.Animation> {
        url = url + fex;
     ...
     */
    public scaleTweena(target: laya.ui.UIComponent, caller: any, duration: number = 500, complete?: Function, props?: any, ease?: Function) {
        target.visible = true;
     ...
     * @param ease ?缓动类型，默认为匀速运动
     */
    public scaleBTweena(target: laya.ui.UIComponent, caller: any, duration: number = 500, complete?: Function, props?: any, ease?: Function) {
     ...
     * @param complete 结束回调函数
     */
    public upToDownTweena(target: laya.ui.UIComponent, props: any, duration: number, caller: any, ease?: Function, complete?: Function) {
...
<td>2</td> <td>使用动态方式，去实时去画（内存开销小，计算开销大，支持换装,不建议使用）</td>
     ...
     */
    public showSkeAnimation(url: string, callBack?: Function, aniMode?: number): void {
        // let self = this;
        console.log(url);
    ...
    /**
         ...
         * @param rate	播放速率
         */
    private show2dBoonAnimationa(url: string, dbBox: Laya.Box, index: number, loop: boolean, rate: number, x: number, y: number, rotation: number) {
    ...
    /**闪烁动画 */
    public flaTweena(target: Laya.Image, caller: any) {
        let alp: number = 0;
    ...
    /**角色头部动画 */
    public swingHeadTweena(target: Laya.Image, caller: any) {
    ...
    /**角色身体动画 */
    public swingBodyTweena(target: Laya.Image, caller: any) {
        let scaleDelta: number = 0;
        Laya.timer.loop(20, caller, () => {
            scaleDelta += 0.04;
    ...
    /**飞金币的动画 */
    public flayGloda(xSrc: number, ySrc: number, xObj: number, yObj: number) {
        let iTeme = 30;
        let nCount = 10;
        let goldArr = [];
     ...
     * @param caller 执行域(this)
     */
    public displayTwinklea(target: Laya.Image | Laya.Button, prefix: string, caller: any) {
        let index = 1;
     ...
     * @param caller 执行域(this)
     */
    public frameAnia(target: Laya.Image, prefix: string, caller: any, frameNum: number, time: number = 100) {
        let index = 1;
    ...
    /**按钮心跳动画 */
    public zoomTweena(target: Laya.Sprite, caller: any, scaleD: number = 0.08) {
        let scaleDelta: number = 0;
        Laya.timer.loop(20, caller, () => {
            scaleDelta += scaleD;
    ...
    public zoomImgTweena(target: Laya.Image, caller: any) {
        let scaleDelta: number = 0;
        Laya.timer.loop(20, caller, () => {
            scaleDelta += 0.04;
    ...
    public titleImgTweena(target: Laya.Image, caller: any) {
        let scaleDelta: number = 0;
        Laya.timer.loop(20, caller, () => {
            scaleDelta += 0.04;
    ...
    /**横向移动 */
    public VTweena(target: Laya.Image, caller: any, ds = 1) {
        let xDelta: number = 0;
        Laya.timer.loop(20, caller, () => {
            xDelta += 0.04;
    ...
    /**横向移动 */
    public HTween(target: Laya.Sprite, caller: any, ds = 1) {
        let xDelta: number = 0;
        Laya.timer.loop(20, caller, () => {
            xDelta += 0.04;
            var xVaule: number = Math.sin(xDelta) * ds;
    ...
    public VTween(target: Laya.Image, caller: any, ds = 1) {
        let yDelta: number = 0;
        Laya.timer.loop(1, caller, () => {
            yDelta += 0.1;
            var yVaule: number = Math.sin(yDelta) * ds;
            target.y += yVaule;
        });
    }
