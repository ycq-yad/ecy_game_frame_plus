...
export default class PlatformDY {
     ...
     * 默认wx 接口url
     */
    public static url: string = "https://zy.qkxz.com/WxApi/?webid=69";//"https://fxqq.xyxapi.com/home/?webid=32";
    //https://fxqq.xyxapi.com/home/?webid=6
    /**qq 接口url  */
    public static qqUrl: string = "https://fxqq.xyxapi.com/home/?webid=18";
    /**qq 接口url  */
    public static ttUrl: string = "https://fxqq.xyxapi.com/home/?webid=32";
    ...
    public static getTTOpenidAndAuthorzia(obj: DYAuthorzia_Cmd): Promise<DYAuthorzia_Rev> {
        let data = UtilsDY.objTPC(obj);

        return new Promise<DYAuthorzia_Rev>((resolve) => {
            HttpManager.getInstance().sendHttpReqDY(PlatformDY.url + "&act=userinfo" + "&", obj, (rev) => {
    ...
    public static getOpenidAndAuthorzia(obj: DYAuthorzia_Cmd): Promise<DYAuthorzia_Rev> {
        return new Promise<DYAuthorzia_Rev>((resolve) => {
            HttpManager.getInstance().sendHttpReqDY(PlatformDY.url + "&act=userinfo&version=" + PlatformDY.version + "&", obj, (rev) => {
                let jsonRev: DYAuthorzia_Rev = rev.data;
                PlatformDY.openid = jsonRev.openid;
                console.log("DY---> authorzia rev = " , rev);
                resolve(jsonRev);
    ...
    public static startGame(): Promise<number> {
        return new Promise<number>((resolve) => {
            HttpManager.getInstance().sendHttpReqDY(PlatformDY.url + "&act=index&version=" + PlatformDY.version + "&openid=" + PlatformDY.openid + "&", null, (rev) => {
    ...
    public static endGame(obj: DYEndGame_Cmd): Promise<number> {
        return new Promise<number>((resolve) => {
            HttpManager.getInstance().sendHttpReqDY(PlatformDY.url + "&act=end&version=" + PlatformDY.version + "&openid=" + PlatformDY.openid + "&", obj, (rev) => {
                let jsonRev = rev.data;
    ...
    public static clickGame(id: string): void {
        HttpManager.getInstance().sendHttpReqDY(PlatformDY.url + "&act=game&version=" + PlatformDY.version + "&id=" + id + "&openid=" + PlatformDY.openid, null, (rev) => {
    ...
    public static toGame(id: string): void {
        HttpManager.getInstance().sendHttpReqDY(PlatformDY.url + "&act=cgame&version=" + PlatformDY.version + "&id=" + id + "&openid=" + PlatformDY.openid, null, (rev) => {
            console.log("DY---> toGame rev = ", rev);
        }, null, "get");
    }

    /**游戏显示列表 */
    public static gameListInfos: any[];
    /**广告显示 */
    ...
    public static getGameList(): Promise<DYGameListItem[]> {
        return new Promise<DYGameListItem[]>((resolve) => {
            ...
            HttpManager.getInstance().sendHttpReqDY(url, null, (rev) => {
